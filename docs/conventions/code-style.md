...
- `make tidy` runs `clang-tidy` against `compile_commands.json`
  (regenerated by `make compdb`). It spawns one process per file, several
  at a time, so a crash costs that one file instead of the whole run; the
  summary names anything that could not be checked and the run then fails.
  `TIDY_JOBS` overrides the parallelism.
